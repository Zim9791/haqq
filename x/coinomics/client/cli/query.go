...
func GetEraTargetSupply() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "era-target-supply",
		Short: "Query era target supply",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			params := &types.QueryEraTargetSupplyRequest{}
			res, err := queryClient.EraTargetSupply(context.Background(), params)
			if err != nil {
				return err
			}
...
func GetTotalTargetSupply() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "total-target-supply",
		Short: "Query total target supply",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			params := &types.QueryTotalTargetSupplyRequest{}
			res, err := queryClient.TotalTargetSupply(context.Background(), params)
			if err != nil {
				return err
			}

			return clientCtx.PrintString(fmt.Sprintf("%v\n", res.TotalTargetSupply))
		},
	}
