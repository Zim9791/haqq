...
SIMAPP = ./app
HTTPS_GIT := https://github.com/haqq-network/haqq.git
...
build-docker:
	...
	$(DOCKER) create --name haqq -t -i ${DOCKER_IMAGE}:latest haqq
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp haqq:/usr/bin/haqqd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/haqq-network/haqq/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(HAQQ_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(HAQQ_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(HAQQ_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(HAQQ_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
